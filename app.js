...
// SVG折れ線グラフの動的描画関数
function renderLineChart(dayGames) {
  const legendContainer = document.getElementById("chartLegend");
  const container = document.getElementById("chartContainer");
  if (!legendContainer || !container) return;
  ...
  PLAYERS.forEach(p => {
    ...
    item.innerHTML = `
      <span class="legend-color-box" style="background-color: ${PLAYER_COLORS[p]};"></span>
      <span>${p}</span>
    `;
  ...
  PLAYERS.forEach(p => {
    cumulativeScores[p] = [0]; // 開始時点0点
  });

  let runningTotal = { "渡邊": 0, "増田": 0, "神山": 0, "坂田": 0, "ゲスト": 0 };
  ...
  // Y軸の最小値・最大値を検出（マージン確保）
  let minScore = 0;
  let maxScore = 0;
  PLAYERS.forEach(p => {
    cumulativeScores[p].forEach(score => {
  ...
  // 座標変換ヘルパー
  const getX = (gameIdx) => paddingLeft + (gameIdx / totalGamesCount) * chartW;
  const getY = (score) => paddingTop + chartH - ((score - minScore) / (maxScore - minScore)) * chartH;
  ...
  for (let i = 0; i <= ySteps; i++) {
    ...
    // Y軸目盛りラベル
    svgHtml += `<text x="${paddingLeft - 8}" y="${yPos + 4}" fill="#94a3b8" font-size="11" text-anchor="end">${Math.round(scoreVal)}</text>`;
  }

  // X軸目盛り ＆ ラベル (局数)
  for (let i = 0; i <= totalGamesCount; i++) {
    const xPos = getX(i);
    const label = i === 0 ? "開始" : `第${sortedGames[i - 1].Game_ID}局`;

    // 縦グリッド線
    svgHtml += `<line x1="${xPos}" y1="${paddingTop}" x2="${xPos}" y2="${paddingTop + chartH}" stroke="#1e293b" stroke-width="1" />`;
    // X軸ラベル
    svgHtml += `<text x="${xPos}" y="${paddingTop + chartH + 20}" fill="#94a3b8" font-size="11" text-anchor="middle">${label}</text>`;
  }
